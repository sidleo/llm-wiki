...
**动机**：本项目是通用知识库，宿主插件**不含任何场景写死的提示词**。各分类可放一个 `APPEND_SYSTEM_PROMPT.md` 声明自己的「行为规则」——描述层每轮把根 + 全部子目录的该文件正文并入 system prompt。例：在 `永辉/sql/` 下放一份写「用了库中没有的表→自动建 Table、踩坑→自动记 Pitfall、新口径先确认」的规则，就实现了该分类的主动记录行为；其他用途的知识库不写它也能用。

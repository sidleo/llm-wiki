...
先查库再动手的约定、该分类的红线（口径 / 命名 / 权限 / 过期与复核）等。插件本身不含任何场景写死的提示词；
各分类的行为完全由这份文件决定。
...
（写该分类特有的红线与约定；例：口径红线、命名规范、权限边界、过期与复核策略。）
...
正文覆盖：先查库再回答（不许凭记忆答库里有的事实与口径）、主动记录（新事实/坑点/确认过的口径）、
写入门控（需 human 确认的类型先征得同意再加 confirmed:true）、收尾 lint、多库切换。
默认库名与路径按当前注册表动态填入。

【通用性约定】文案只讲**通用机制**（怎么查、怎么写、门控、渐进披露），触发条件按「知识的类型」描述；
某个**具体领域**的触发条件（表/字段/取数口径、接口约定、审批流程…）属于数据，
写在库内各目录的 \`APPEND_SYSTEM_PROMPT.md\` 里随库分发，绝不写进这份文案——否则换个领域的库就会被误导。

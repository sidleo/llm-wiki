...
version: 0.4.15
description: >-
  工作知识库（OKF v0.2 markdown bundle，用 `wiki` CLI 读写）：收录某个团队/领域**反复要用到的知识**
  ——事实与结构、约定与口径、流程与规范、经验与坑点。**以下情况先用本 skill 查库，不要凭记忆或印象回答**：
  要引用既有事实或结构（举例：表与字段、接口与字段约定、系统/资产清单、术语表、组织架构）；
  要遵守既有约定或口径（举例：指标怎么算、命名规则、流程步骤、权限边界、审批要求）；
  动手前有既有做法与红线（举例：写 SQL、写代码、配环境、出报表、发布前要遵守的既有做法）；
  出现对不上、算出来不一致、报错或异常需要排查；用户提到「按我们的规定」「上次那个坑」「库里的口径」；
  需要沉淀经验（踩坑、业务规则、探查结论、新发现的事实）；用户提到知识库、经验库、表口径文档、取数经验、
  SQL 知识库、接口文档、流程规范、踩过的坑、llm-wiki、OKF、Open Knowledge Format、knowledge bundle、
  agent knowledge；要把外部资料收进知识库、体检知识库（断链/过期/索引）、或在多台机器/多人之间同步知识库。
  不负责：与知识库无关的通用编程问答与闲聊；具体数据本身由数据平台工具查询，本 skill 提供的是
  「事实是什么、口径怎么定、有哪些坑」这类知识。
metadata:
  requires:
    bins: ["wiki"]
  cliHelp: "wiki --help"
...
**先查库、再动手**。以下情况**必须**先查库，不许凭记忆答（下列为**领域举例**，触发条件按「知识的类型」判断，你库里实际收录什么以 `wiki list` 与目录规则为准）：

- 要引用**既有事实或结构**（字段/接口/清单/术语表/组织架构）→ `wiki search <关键词>`，命中就读 `wiki get <id>` 全文
- 要遵守**既有约定或口径**（算法、命名、流程、权限、过滤与时间范围）→ 先 `wiki search`；没命中就先实际探查再回答，不臆造
- **动手之前**（写 SQL / 写代码 / 配环境 / 出报表 / 发布）→ 先读该分类的**规则与红线**；读到的坑点（Pitfall）必须遵守
- **对不上 / 不一致 / 报错要排查** → 先查同类踩坑记录，再排查、再下结论
- 用户说「按我们的规定」「上次那个坑」「库里的口径」→ 先查库

**主动记录**（不必等用户吩咐；细节规则以 `wiki rules` 的输出为准）：

- 用到库里没有的**事实/结构**（新表、新接口、新系统、新术语）→ 探查后用 `wiki create` 补录
- 踩了坑 / 查出根因 → 记 `Pitfall`；与用户确认过的口径/约定 → 记 `Metric` 或 `Attested Computation`
...
| `wiki prompt [--raw] [--full] [--mcp]` | 生成「宿主常驻要求」文本（贴进宿主的自定义指令/系统提示词）；`--mcp` 输出 MCP 工具版文案，见 `wiki help prompt` |

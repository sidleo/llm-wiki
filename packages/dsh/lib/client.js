...
`;
		...
		async function api(path, init) {
			...
			if (!res.ok || body && body.ok === false) throw new Error(body && body.error || `${path} 请求失败（HTTP ${res.status}）`);
			return body;
		}
		...
		function Field(props) {
			return e("label", { className: "dwField" }, e("span", { className: "dwLabel" }, props.label), e("input", {
				className: "dwInput",
		...
		function BundleRow(props) {
			...
			return e("li", { className: "dwRow" }, e("input", {
			...
			}), e("div", { className: "dwRowMain" }, renaming ? e("input", {
				className: "dwInput",
				value: newName,
				disabled: props.busy,
				...
				className: "dwBtn",
				disabled: props.busy,
				onClick: () => props.onRename(b.name, newName)
			}, "确定"), e("button", {
				key: "cancel",
				className: "dwBtn",
				onClick: () => {
		...
		function Card() {
			...
			const syncSeq = react.default.useRef(0);
			...
			const [syncLoading, setSyncLoading] = react.default.useState(false);
			...
			const [cloneDir, setCloneDir] = react.default.useState("");
			const [cloneName, setCloneName] = react.default.useState("");
			const [cloneUse, setCloneUse] = react.default.useState(false);
			const [showClone, setShowClone] = react.default.useState(false);
			const run = react.default.useCallback(async (fn) => {
				setBusy(true);
				setErr("");
				try {
					await fn();
			...
			const loadSync = react.default.useCallback(async (bundle) => {
				...
				try {
					...
					if (syncSeq.current === seq) setSyncInfo({
						...r,
						forBundle: bundle
					});
				} catch (error) {
			...
			if (!open) {
				...
				return e("li", { className: "dwCard" }, e("button", {
					type: "button",
					className: "dwHeader",
					"aria-expanded": "false",
					onClick: () => setOpen(true)
				}, e("span", { className: "dwHeadText" }, e("span", { className: "dwName" }, "llm-wiki 知识库"), e("span", { className: "dwDesc" }, summary)), e(Chevron, { open: false })));
			}
			return e("li", { className: "dwCard dwCardOpen" }, e("button", {
				type: "button",
				className: "dwHeader",
				"aria-expanded": "true",
				onClick: () => setOpen(false)
			}, e("span", { className: "dwHeadText" }, e("span", { className: "dwName" }, "llm-wiki 知识库"), e("span", { className: "dwDesc" }, state ? `默认：${state.active.name}（${state.active.path}）` : "正在读取…")), e(Chevron, { open: true })), e("div", { className: "dwBody" }, err ? e("div", { className: "dwWarn" }, `✕ ${err}`) : null, notice ? e("div", { className: "dwOk" }, `✓ ${notice}`) : null, busy ? e("div", { className: "dwHint" }, "执行中…（首次同步要拉取上百个文件，约 1 分钟；请勿关闭页面，完成后自动刷新状态）") : null, e(Section, {
				title: "命名目录（bundle）",
				hint: `写入 ${state ? state.registryPath : "~/.agents/wiki-registry.json"}，与 CLI / pi 形态共享；删除只摘除注册，不动磁盘数据。`
			}, state ? e("ul", { style: {
				listStyle: "none",
				margin: 0,
				padding: 0
			} }, state.entries.length ? state.entries.map((b) => e(BundleRow, {
				key: b.name,
				bundle: b,
				busy,
				confirmingRemove,
				onActivate: (name) => run(async () => {
					await post("/bundles", {
						op: "activate",
						name
					});
					await afterMutation(`已把「${name}」设为默认`);
				}),
				onRename: (name, newName) => run(async () => {
					await post("/bundles", {
						op: "rename",
						name,
						newName,
						path: state.entries.find((x) => x.name === name).path
					});
					await afterMutation(`已改名为「${newName}」`);
				}),
				onRemove: (name) => run(async () => {
					await post("/bundles", {
						op: "remove",
						name
					});
				...
				onClick: () => loadSync(viewBundle)
			}, syncLoading ? "读取中…" : "刷新状态")), isFeishu ? e("div", null, syncLoading && !sync ? e("div", { className: "dwHint" }, "正在读取飞书状态…（列远端目录，通常 2–5 秒）") : null, sync && sync.feishu && sync.feishu.ok ? e("div", null, e("div", { className: "dwHint" }, `待推送 ${sync.feishu.counts.push}｜待拉取 ${sync.feishu.counts.pull}｜冲突 ${sync.feishu.counts.conflict}｜本地 ${sync.feishu.counts.local} 个 .md，远端 ${sync.feishu.counts.remote} 个 .md`), activeEntry && activeEntry.folderUrl ? e("div", { className: "dwHint" }, `文件夹：${activeEntry.folderUrl}`) : null, activeEntry && activeEntry.cacheDir ? e("div", { className: "dwHint" }, `本地缓存：${activeEntry.cacheDir}`) : null, sync.feishu.ignored && sync.feishu.ignored.length ? e("div", { className: "dwHint" }, `已忽略的非 .md 资源：${sync.feishu.ignored.slice(0, 5).join("、")}`) : null) : null, sync && sync.feishu && sync.feishu.conflict && sync.feishu.conflict.length ? e("div", { className: "dwWarn" }, `冲突（两侧都改，需人工处理）：${sync.feishu.conflict.map((x) => x.rel).join("、")}`) : null, sync && sync.feishu && sync.feishu.duplicates && sync.feishu.duplicates.length ? e("div", { className: "dwWarn" }, `⚠ 远端同名重复：${sync.feishu.duplicates.map((x) => x.rel).join("、")}——飞书允许重名，工具不替你挑；请在飞书里删掉多余的那个（涉及该目录的推送会停下报错）。`) : null, sync && sync.feishu && sync.feishu.remoteDeleted && sync.feishu.remoteDeleted.length ? e("div", { className: "dwHint" }, `远端已删（本地保留）：${sync.feishu.remoteDeleted.join("、")}`) : null, !syncLoading && !(sync && sync.feishu && sync.feishu.ok) ? e("div", null, e("div", { className: "dwHint" }, sync && sync.feishu && sync.feishu.error || "飞书状态不可用（需要 lark-cli 已登录）"), sync && sync.feishu && sync.feishu.next ? e("div", { className: "dwHint" }, sync.feishu.next) : null) : null, e("div", { style: {
				display: "flex",
				gap: 8,
				flexWrap: "wrap",
				marginTop: 8
				...
				className: "dwBtn dwBtnPrimary",
				disabled: busy || syncLoading || !(sync && sync.feishu && sync.feishu.ok),
				onClick: () => run(async () => {
					const r = await post("/sync", {
						op: "sync",
						bundle: viewBundle
				...
				target: "_blank",
				rel: "noreferrer"
			}, "在飞书中打开") : null)) : e("div", null, state && state.git && state.git.ok === false ? e("div", { className: "dwWarn" }, `✕ ${state.git.error || "git 不可用"}`) : null, sync && sync.git ? sync.git.ok ? e("div", null, e("div", { className: "dwHint" }, `远端：${sync.git.remote || "（未配置）"}｜分支：${sync.git.branch || "(detached)"}${sync.git.upstream ? ` → ${sync.git.upstream}` : ""}｜领先 ${sync.git.ahead} / 落后 ${sync.git.behind}｜改动 ${sync.git.dirty.length} 个文件`), sync.git.lastCommit ? e("div", { className: "dwHint" }, `最后提交：${sync.git.lastCommit}`) : null, sync.git.conflicts && sync.git.conflicts.length ? e("div", { className: "dwWarn" }, `存在冲突：${sync.git.conflicts.join("、")}`) : null) : e("div", null, e("div", { className: "dwHint" }, sync.git.error || "git 状态不可用"), sync.git.next ? e("div", { className: "dwHint" }, sync.git.next) : null) : e("div", { className: "dwHint" }, syncLoading ? "正在读取 git 状态…" : "（未读取到状态，点「刷新状态」重试）"), e("div", { style: {
				display: "flex",
				gap: 8,
				flexWrap: "wrap",
				marginTop: 8
			...
			}, sync && sync.git && sync.git.ok ? "立即同步" : "重试同步"), sync && sync.git && sync.git.ok ? null : e("button", {
				className: "dwBtn",
			...
			}, "初始化远端并首推")), sync && sync.git && sync.git.ok ? null : e(Field, {
				label: "远端 URL（init 用）",
				value: remoteUrl,
				...
				disabled: busy
			}, showClone ? "收起「克隆远端」" : "克隆远端到本地…"), showClone ? e("div", null, e(Field, {
				label: "远端 URL",
				value: cloneUrl,
				disabled: busy,
				onChange: setCloneUrl
			}), e("div", { className: "dwGrid" }, e(Field, {
				label: "本地目标目录",
				value: cloneDir,
				disabled: busy,
			...
			})), e("label", { className: "dwCheck" }, e("input", {
				type: "checkbox",
				checked: cloneUse,
				disabled: busy,
				onChange: (ev) => setCloneUse(ev.target.checked)
			}), "注册后设为默认分支"), e("button", {
				className: "dwBtn",
				...
				disabled: busy,
				onClick: () => run(async () => {
					setHealth(await api(`/health?bundle=${encodeURIComponent(viewBundle)}`));
				})
			}, "运行体检"), e("button", {
				className: "dwBtn",
				disabled: busy,
				onClick: () => run(async () => {
					const r = await post("/index", { bundle: viewBundle });
					setNotice(`已重建 index：${r.files.length} 个文件`);
				})
			}, "重建 index")), health ? e("div", null, e("div", { className: "dwHint" }, `目录 ${health.tree.dirs} 个｜概念 ${health.tree.concepts} 个｜OKF ${health.validate.ok ? "合规 ✓" : `不合规（${health.validate.errors.length} 错 / ${health.validate.warnings.length} 警）`}｜断链 ${health.lint.summary.broken}｜过期 ${health.lint.summary.stale}｜孤儿 ${health.lint.summary.orphans}｜问题合计 ${health.lint.issues.length}`), health.validate.errors.length ? e("div", { className: "dwWarn" }, health.validate.errors.slice(0, 5).join("；")) : null, health.lint.issues.length ? e("div", { className: "dwPre" }, health.lint.issues.slice(0, 60).map((i) => `[${i.kind}] ${i.msg}`).join("\n")) : e("div", { className: "dwOk" }, "lint clean ✓")) : null)));
		}
		function apply(ctx) {
